...
class trainingModel(Module):
    ...
    def trainModel(self, tr_set, val_set, batch_size, parameter_funct=Module.parameters, num_epochs=100, patience=5, optimizer=Adam, learning_rate=1e-3, file_path=None):
        if file_path == None:
        ...
        for epoch in range(self.trainedEpochs, num_epochs):
            ...
            for (batch_data, batch_y) in tqdm(tr_set, desc='Training: '):
                ...
                optimizer.zero_grad()
                _, tr_loss = model(batch_data, batch_y)
                #cuda.empty_cache()
                tr_loss.backward()
                optimizer.step()
            ...
            with no_grad():
                for (batch_data, batch_y) in tqdm(val_set, desc='Validation: '):
                    ...
                    batch_data = batch_data.to(choosedDevice)
                    batch_y = batch_y.to(choosedDevice)
                    _, val_loss = model(batch_data, batch_y)


            self.trainedEpochs = epoch + 1
            self.trLossList.append(tr_loss)
