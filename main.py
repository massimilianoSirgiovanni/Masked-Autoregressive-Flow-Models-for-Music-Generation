...
if exists("./savedObjects/models/MAF_model"):
    ...
    choosedModel.parametersInitialization((len(notes), 32), hidden_sizes=[50, 100, 80], n_layers=3, device=device, madeType="univariate")
    print(choosedModel)
    trainObj = trainingModel(choosedModel)


print(notes)
trainObj.trainModel(tr_set[:, :, notes], val_set[:, :, notes], batch_size=1000, loss_function=loss_function, num_epochs=200, patience=20, learning_rate=0.01, file_path="./savedObjects/models/MAF_model")

trainObj.plot_loss()

trainObj.testModel(tr_set[:, :, notes], set_name="Training Set")
trainObj.testModel(te_set[:, :, notes], set_name="Test Set")
